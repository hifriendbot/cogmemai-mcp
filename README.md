...
This repository is the documentation home for the `cogmemai-mcp` npm package.

**The current release is [v3.21.0 on npm](https://www.npmjs.com/package/cogmemai-mcp)**, published from a private source tree. Install it with `npx cogmemai-mcp setup`. The package is distributed as a compiled build.

The files under `src/` in this repository are the **v2.0.0 reference implementation**, kept for reference only. They are not synchronized with the current release, and CogmemAi's retrieval and intelligence internals are not open source. For what actually ships today, see the release notes below, the [npm version history](https://www.npmjs.com/package/cogmemai-mcp?activeTab=versions), and the [developer documentation](https://hifriendbot.com/developer/).
